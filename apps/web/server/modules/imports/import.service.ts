...
Analizza il testo estratto da un ${docLabel} e restituisci SOLO un oggetto JSON valido con questa struttura:
{
  ...
  "plantName": "string opzionale",
  "measurementDate": "YYYY-MM-DD opzionale",
  "technicianName": "string opzionale",
  "laboratoryName": "string opzionale",
  "parameters": [
...
- Usa codici parametro standard se riconoscibili (COD, BOD, pH, SS, NTU, ecc.).
...
export async function createPdfImport(params: {
  ...
  const extractedText = extractPdfText(params.buffer);
  ...
  const resolvedCustomerId = await resolveCustomerId(params.customerId, preview);
  if (!resolvedCustomerId) {
    throw new Error(
      params.customerId
        ? "Cliente non trovato"
        : "Seleziona un cliente oppure carica un PDF con ragione sociale leggibile"
    );
  }
